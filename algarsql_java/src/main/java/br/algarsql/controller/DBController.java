...
public class DBController {
    ...
    public Map<String, Object> dbExecute(HttpServletRequest request, HttpServletResponse response,
            HttpSession session) {
        ...
        try {
            ...
            if (action.equals("connect")) {
                db = DATABASE.createConnection(request.getParameter("usr").toString(),
            ...
            if (action.equals("change_password_get_user")) {
                String[] msg =
                        Utils.changePasswordGetUser(request.getParameter("alias"), o.toString());
            ...
            if (action.equals("save_row_grid")) {
                ...
                db.executeUpdateFromPrimaryKey(rowid, sql, itens);
            }
            ...
            if (action.equals("test_procedure")) {
                String ret_proc =
                        db.createProcedureTest(request.getParameter("object_name").toString());
                ret.put("status_code", 0);
            ...
            if (action.equals("ddl")) {
                String[] x = request.getParameter("object_name").toString().split("\\.\\.\\.");
            ...
            if (action.equals("workdata_import")) {
                String sql = Utils.compactSQL(request.getParameter("sql").toString());
                new ParallelProcess(4, db, sql).start();
                ret.put("status_code", 0);
                ret.put("status_msg", "Import started successfully");
                return ret;
            }
