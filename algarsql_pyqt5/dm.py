...
from PyQt5.QtGui import QPalette, QColor, QIcon, QPixmap, QFont, QKeySequence
from PyQt5.QtCore import Qt, QSize, QObject, pyqtSignal, QThread
from PyQt5.QtWidgets import QMessageBox, QPushButton, QMenu, QAction, QTableWidget, QTableWidgetItem, QApplication
...
def find_regex(text, pattern, default=None):
    ...
    if m:
        return m.group(1)
    return text if default == None else default
...
class ORACLE:
    ...
    def SELECT(self, p_sql, direct=False, logger=False, fetchSize=None):
        ...
        try:
            ...
            for idx, t in enumerate(self.col_types):
                print( f"{self.col_names[idx]} : {t}" )
