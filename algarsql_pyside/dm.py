...
from PySide6.QtGui import QPalette, QColor, QIcon, QPixmap, QFont, QKeySequence
from PySide6.QtCore import Qt, QSize, QObject, Signal, QThread
from PySide6.QtWidgets import QMessageBox, QPushButton, QMenu, QTableWidget, QTableWidgetItem, QApplication
...
def moveToCenter(widget, size_widget=None):
    size   = widget.size() if size_widget is None else size_widget
...
def createButtonWork(Run=None, Size=QSize(600, 100), Text="wait a few seconds...", Frameless=True):
    ...
    G.setText(Text)
    G.clicked.connect(Run if Run is not None else lambda: (G.deleteLater()))
    moveToCenter(G, Size)
    G.show()
    return G


def createDataGrid(owner, fields_data, fields_types, size=QSize(800, 600)):
    owner.G = QTableWidget()
    owner.G.setFixedSize(size)
    owner.G.setWindowFlag(Qt.WindowStaysOnTopHint)
    populateGrid(owner.G, fields_data, fields_types)
    moveToCenter(owner.G)
    owner.G.show()    
...
def populateGrid(grid: QTableWidget, data, columnNames=None, columnTypes=None, editableColumns="", appending=False, columnWidth=200):
    row_idx = 0 if not appending else grid.rowCount()
    ...
    for i, x in enumerate(data):
        for j, a in enumerate(x):
            item = QTableWidgetItem()
            if f"-{j}-" not in editableColumns:
                item.setFlags(item.flags() ^ Qt.ItemIsEditable)
            if a is None:
                tt = ""
            elif columnTypes is not None and "DATE" in columnTypes[j]:
                tt = a.strftime("%d/%m/%Y %H:%M:%S")
            elif columnTypes is not None and "BLOB" in columnTypes[j]:
...
def find_regex(text, pattern, default=None):
...
## ==============================================================================================
## CLASSE ORACLE (sem alterações de lógica)
## ==============================================================================================
