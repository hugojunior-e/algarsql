...
class form(QWidget):
    def __init__(self):
        ...
        self.mem_editor_popup = dm.MENU([
            "Find|Ctrl+F", "-", "Format SQL|Ctrl+Alt+F", "UPPERCASE|Ctrl+U", "lowercase|Ctrl+L", "-",
            "Paste with UNION|-", "Paste with ITENS|-", "-", "Comment|-", "Uncomment|-", "-",
            "Describe|-", "Properties|-", "Drop Table|-", "-"
        ], self.mem_editor_popup_click, self)
        ...
        self.ui.bt_fetch.setText("⮟")
        self.ui.bt_fetch.setEnabled(False)
        ...
        self.editorSQL = EDITOR_SQL(text="SELECT * FROM DUAL", customContextMenu=self.mem_editor_popup_show)
        ...
        self.ui.bt_tool_descr_sql.clicked.connect(self.bt_tool_show_columns_clicked)
    ...
    def tabTextIcon(self, Text=None, Icon=None, changed=""):
        ...
        if Text is not None:
            ...
            nome1 = "" if self.filename is None else os.path.basename(self.filename) + changed
            nome2 = "" if self.objectname is None else "@" + self.objectname
    ...
    def mem_editor_popup_show(self, position):
        txt = self.editorSQL.getSelectedText() or ""
    ...
    def mem_editor_popup_click(self):
        ...
        if self.sender().text() == "Find":
            ...
            x = "\nUNION ALL\n".join([f"SELECT '{d.strip()}' o from dual" for d in dm.clipboard.text().strip().split("\n")])
            self.editorSQL.replaceSelectedText(x)
            ...
            if dm.db.status_code == 0:
                dm.createDataGrid(self, dm.db.col_data, dm.db.col_names)
        elif self.sender().text() == "Properties":
            x = dm.db.SELECT(p_sql=dm_const.C_SQL_PROPERTIES % (txt.upper()), fetchSize=0)
            if dm.db.status_code == 0:
                col_data = [ (dm.db.col_names[i], info) for i, info in enumerate(dm.db.col_data) ]
                dm.createDataGrid(self, col_data, ["Property", "Value"])
            else:
    ...
    def bt_tool_show_columns_clicked(self):
        dm.createDataGrid( self, [ [ self.db.col_names[i], self.db.col_types[i] ] for i,_ in enumerate(self.db.col_types) ],  ["FieldName", "FieldType"], )
    ...
    def executeSQL(self):
        ...
        self.ui.bt_fetch.setEnabled(self.db._sql_type == 1)
