...
class form(QDialog):
    ...
    def tree_tns_itemclicked(self, it):
        current = self.ui.tree_tns.currentItem()
        if current and current.text(1):
            self.ui.edt_username.setText(current.text(1))
            self.ui.edt_password.setText(current.text(2))
            self.ui.cbo_database.setCurrentText(current.text(3))

    def bt_conectar_clicked(self):
        dm.db.CONNECT(
            self.ui.edt_username.text(),
            self.ui.edt_password.text(),
            self.ui.cbo_database.currentData(),
            self.ui.chk_direct_access.isChecked(),
        )

        if dm.db.status_code == 0:
            for i in range(dm.f_principal.ui.pc_editor.tabBar().count()):
                editor = dm.f_principal.ui.pc_editor.widget(i)
                editor.db.disconnect()
                editor.ui.chk_run_user_local.setVisible(not dm.db.is_direct)

            dm.f_principal.setWindowTitle(
                f"{self.ui.edt_username.text()}@{self.ui.cbo_database.currentText()} - ({dm.db.login_banner})"
            )
