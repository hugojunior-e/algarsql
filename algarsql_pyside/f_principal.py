...
import sqlite3
import f_editor_tti
from PySide6.QtWidgets import QMainWindow, QTreeWidgetItem, QFileDialog, QFileSystemModel
...
class form(QMainWindow):
    def __init__(self):
        ...
        self.actionConfigs_popup = dm.MENU(
            ["Recall SQL|Ctrl+E", "View Sessions|-", "Find Objects|-", "Explain Query|F5",
             "CSV Updater|-", "-", "Recompile Invalid Objects|-", "-", "Reload Templates|-",
             "-", "Config Tools|-", "-", "Export data Table|-"],
            self.actionConfigs_popup_click, self
        )

        self.tree_objetos_popup = dm.MENU(
            ["View Code|-", "Query Data|-"],
            self.tree_objetos_popup_click, self
        )
        ...
        self.ui.actionConfigs.triggered.connect(lambda: self.actionConfigs_popup.exec(QCursor.pos()))
        ...
        self.ui.pc_editor.removeTab(0)
        self.ui.pc_editor.tabCloseRequested.connect(
            lambda index: (self.ui.pc_editor.widget(index).db.disconnect(),
                           self.ui.pc_editor.removeTab(index))
        )
    ...
    def extrai_ddl(self, owner, type, name):
        ...
        self.bt = dm.createButtonWork(Run=lambda: (self.th.thread.terminate(), self.bt.close()),
                                      Text=f"Extracting DML {owner + '.' + name}")
        self.bt.info = owner + '.' + name
        self.th = dm.WORKER(proc_run=lambda: (dm.db.DDL(owner, type, name)),
                            proc_fim=self.tree_objetos_popup_view_code_finish)
    ...
    def tree_objetos_popup_show(self, position):
        ...
        if x:
            ...
            self.tree_objetos_popup.exec(self.ui.tree_objetos.viewport().mapToGlobal(position))
    ...
    def tree_objetos_popup_click(self):
        ...
        elif self.sender().text() == "Query Data":
            xx = self.ui.tree_objetos.currentItem().text(1).split("/")
            tab = self.actionNewEditor_click()
            tab.editorSQL.setText(f"SELECT * FROM {xx[0]}.{xx[2]}")
            tab.executeSQL()
    ...
    def tree_objetos_montar(self):
        ...
        self.th1 = dm.WORKER(proc_run=self.tree_objetos_montar_start,
                             proc_fim=lambda: self.ui.tree_objetos.addTopLevelItems(
                                 self.ui.tree_objetos.__topLevelItems))
    ...
    def actionOpenEditor_click(self):
        fileName, _ = QFileDialog.getOpenFileName(
            None, "Open File", "", "SQL Files(*.sql);;All Files (*)",
    ...
    def actionSaveEditor_click(self):
        ...
        novo = False
        if tab:
            if tab.filename is None:
                nome, _ = QFileDialog.getSaveFileName(
                    self, "Save file", "", "SQL Files (*.sql)",
                    options=QFileDialog.Option.DontUseNativeDialog
                )
                novo = True
            else:
    ...
    def popup_config_recompile(self):
        ...
        if dm.db.status_code == 0:
            ...
            for x in dm.db.col_data:
                ...
                self.bt.H_msg_ret += [x[0], dm.db.status_msg, "----------------------------------------"]
        self._bt_.close()
    ...
    def popup_config_export_data(self):
        ...
        sqlite_cursor.execute(f"CREATE TABLE IF NOT EXISTS tabela ({', '.join(columns)})")
        insert_query = f"INSERT INTO tabela VALUES ({', '.join(['?' for _ in columns])})"
        qtd = 0
        while self.bt.isVisible():
            ...
            dados = []
            for xx in row:
                dados_item = [item.read() if "LOB" in str(type(item)).upper() else item for item in xx]
                dados.append(dados_item)
            sqlite_cursor.executemany(insert_query, dados)
            sqlite_conn.commit()
    ...
    def actionConfigs_popup_click(self):
        ...
        if stext == "Reload Templates":
            ...
            dm.f_editor.showForm(stext, "" if ff is None else ff.editorSQL.getText())
    ...
    # ==============================================================================================
    # tabchange + find/replace
    # ==============================================================================================
    def pc_editor_tabchange(self):
        try:
            w = self.ui.pc_editor.currentWidget()
            in_exec = w.db.in_execution
            in_tran = w.db.in_transaction
            self.ui.actionRunSQL.setEnabled(not in_exec)
            self.ui.actionStopSQL.setEnabled(in_exec)
            self.ui.actionCommit.setEnabled(in_tran)
            self.ui.actionRollback.setEnabled(in_tran)
        except Exception:
    ...
    def findReplaceActions(self, action):
        ...
        if frm:
            frm.editorSQL.finder_prepare([
                self.ui.edt_find_text.text(),
                self.ui.edt_find_replace.text(),
                self.ui.chk_find_match_case.isChecked(),
                self.ui.chk_find_match_whole_word.isChecked()
            ])
            if action == 1:
