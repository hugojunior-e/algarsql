...
function js_editor_mode_open(object_name) {
    change_icon(true);
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "ddl", "object_name": object_name }, function (a) {
        if ( a.status_code != 0 ) {
            alert(a.status_msg);
            return;
        }
...
function js_global_thread_status(id_logger, id_logger_text) {
    ...
    ajax("/th_status", {}, function (a) {
        id_logger_text.innerHTML = a.global_th_ret;
        if ( a.global_th_stop == false  ) {
            setTimeout(() => {
...
function js_csv_completer_execute() {
    ...
    formData.append("file_data", input.files[0]); // 👈 aqui vai o arquivo
...
function js_logoff_form() {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "logoff" }, function (a) {
        change_icon();
    });    
}
...
function js_login_connect() {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "connect", "usr": id_login_username.value, "pwd": id_login_password.value, "tns": id_login_database.value, "direct": id_login_direct.value }, async function (a) {
        change_icon(false);
...
function js_db_execute_proc( obj ) {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "test_procedure", "object_name": obj }, function (a) {
...
function js_db_grid_editrow_save() {
    ...
    params = {
        "session_id": global_var.session_id,
...
function js_db_stop() {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "stop", }, function (a) {
        change_icon(false);
    }, function (error) {
        change_icon(false);
    });
...
function js_db_explain() {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "explain", "sql": get_sql_editor() }, function (a) {
        if ( a.status_code != 0 ) {
            alert(a.status_msg);
            return;
        }
...
function js_db_transaction(action) {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "action", action }, function (a) { 
...
function js_db_describe() {
    ...
    if (global_var.object_tables.includes(obj.toUpperCase())) {
        ajax("/db_execute", { "session_id": global_var.session_id, "action": "describe", "object_name": obj },
            function (a) {
...
function js_db_fetch50(grid) {
    ajax("/db_execute", { "session_id": global_var.session_id, "action": "fetch", }, function (a) {
        if (a.data.length > 0) {
        ...
        monaco.languages.registerCompletionItemProvider('sql', {
            ...
            async provideCompletionItems(model, position) {
                ...
                const suggestions = (columns || []).map(col => ({
                    ...
                    insertText: col[0] + ( in_list_table ? "" : " " + gerar_alias(col[0]) )
                }));
