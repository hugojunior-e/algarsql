...
class form(QWidget):
    ...
    def popup_editor_item_click(self):
        ...
        if self.sender().text() == "Describe":
            x = dm.db.executeSQL(p_sql=dm.C_SQL_DESCRIBE % (txt.upper()))
        ...
        if self.sender().text() == "Properties":
            x = dm.db.executeSQL(p_sql=dm.C_SQL_PROPERTIES % (txt.upper()))
            if dm.db.status_code == 0:
                col_data = []
        ...
        try:
            ...
            if event.key() == 46:
                ...
                if tc.selectedText().upper() in dm.all_users:
                    dm.db.executeSQL(p_sql=dm.C_SQL_ALL_TABLES % (tc.selectedText()))
                    lista = [x[0] for x in dm.db.cur.fetchall()]

                elif tc.selectedText().upper() in dm.all_tables:
                    dm.db.executeSQL(p_sql=dm.C_SQL_ALL_TAB_COLUMNS % (tc.selectedText()))
                    lista = [x[0] for x in dm.db.cur.fetchall()]
                    ...
                    for i, info in enumerate(info_a):
                        if tc.selectedText().upper() == info.upper() and info_a[i-1] in dm.all_tables:
                            dm.db.executeSQL(p_sql=dm.C_SQL_ALL_TAB_COLUMNS % (info_a[i-1]))
                            lista = [x[0] for x in dm.db.cur.fetchall()]
                            break
    ...
    def th_process_exec_grid(self):
        if self.objectname != None:
            if self.db.status_code != 0:
                ...
                self.db.executeSQL(p_sql=dm.C_SQL_ALL_ERRORS % (ii[0], ii[1]), p_tipo='SELECT_DIRECT' )
